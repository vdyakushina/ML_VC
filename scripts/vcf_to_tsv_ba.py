...
def process_vcf(vcf_fn):
    ...
    for v in in_vcf:
        ...
        if 'INDEL' in in_vcf_fn:
          info_SC_TO_FILTER = [x.split('=')[-1] for x in variant_info_all if 'Scalpel_TO_FILTER' in x]
          info_SC_TNP_FILTER = [x.split('=')[-1] for x in variant_info_all if 'Scalpel_TNP_FILTER' in x]
          ...
          variant_table.append(
              {
                  ...
                  'Scalpel_TO_FILTER': info_SC_TO_FILTER[0] if len(info_SC_TO_FILTER) == 1 else 'N/A',
                  'Scalpel_TNP_FILTER': info_SC_TNP_FILTER[0] if len(info_SC_TNP_FILTER) == 1 else 'N/A',
                  'SINVICT': info_SINVICT[0] if len(info_SINVICT) == 1 else '0',
...
if __name__ == '__main__':
    ...
    if 'vcf.gz' in in_vcf_fn[-7:]:
        out_tsv_fn = in_vcf_fn.split('.notcommon')[0][:-7] + '.tsv'
        # out_tsv_fn = in_vcf_fn[:-7] + '.tsv'
    else:
        out_tsv_fn = in_vcf_fn.split('.notcommon')[0][:-4] + '.tsv'
        # out_tsv_fn = in_vcf_fn[:-4] + '.tsv'
    table.to_csv(out_tsv_fn, sep='\t', header=True, index=False)

    run("rm %s*" % in_vcf_fn, shell=True)

    # wildcard_fn = '*raw*.vcf'
    # tables = []
    # count_main = 0
    # for vcf_fn in glob.glob(in_vcf_fn + '/' + wildcard_fn):
    #     print('VCF', str(count_main) + ':', vcf_fn)
    #     table = process_vcf(vcf_fn)
    #     print(table)
    #     tables.append(table)
    #     count_main += 1
