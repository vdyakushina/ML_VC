...
(variant, f, n) = sys.argv[1:4]

cond={0.05:[500, 1500], 0.1:[500, 1500], 0.25:[250, 1000], 0.5:[250, 1000]}

DP=[]
i=0
while i < int(n):
	DP.append(np.random.randint(cond[float(f)][0], cond[float(f)][1]))
	i+=1
...
def run_func(depth):
	...
	folder=variant.replace('>','_').replace(':','_')+'_'+str(f)+'_'+str(depth)
...
pool = mp.Pool(4)
pool.map(run_func, [depth for depth in DP])
pool.close()
