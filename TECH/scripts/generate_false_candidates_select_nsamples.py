...
samples=[]
with open('/home/gkhvorykh/samples/meta.csv', 'r') as inp_s:
  for line in inp_s:
...
vars=[]
with open('/home/gkhvorykh/train_false/false_SNV.tsv', 'r') as inp_f:
  for line in inp_f:
...
while i < var_n and len(samples_s)>0:  
  ...
  samples_s.remove(sample)
  tsv=pd.read_csv('/home/gkhvorykh/samples/%s/analysis/variant.raw.SNV.tsv' % sample, sep='\t')
  tsv=tsv[(tsv['Mutect2_TO_FILTER']>=MutF_bottom)]
  if len(tsv) > 0:
    select=random.randint(0,len(tsv)-1)
    variant=tsv.iloc[select]['variant_name']
    print(variant)
...
vars=[]
with open('/home/gkhvorykh/train_false/false_INDEL.tsv', 'r') as inp_f:
  for line in inp_f:
    vars.append(line.split('\t')[2])
